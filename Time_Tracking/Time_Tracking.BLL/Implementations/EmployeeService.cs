...
public class EmployeeService : IEmployeeService
{
    ...
    public async Task<UserManagerResponse> ImportPendingTasksAsync(string employeeId)
    {
        ...
        var overDueTasks = overdueTasks.ToList();
        var undoneTasks = pendingTasks.ToList();
        foreach (var task in overDueTasks.Concat(undoneTasks))
        {
            task.State = task.State == State.InProgress ? State.Paused : State.NotStarted;
        ...
        var todoDtos = overDueTasks.Concat(undoneTasks)
